...
void MainWindow::on_btn_door_1_clicked()
{
    ...
    if (userName == nullptr) {
        ...
        QString dataFile = readFileOpenError(pathFileOpenErrorDoor1);
        ...
        if (date.secsTo(QDateTime::fromString(fileDateTime, "yyyy.MM.dd hh:mm:ss")) >= -30){
            writeFileOpenError(pathFileOpenErrorDoor1, formattedTime, count);

            if (count.toInt() > 3) {
                QString noti = "Thông báo (door_1)";
                QString text = "Có người đang mở cửa với thẻ không hợp lệ";

                QMessageBox::warning(this, noti, text);
            }
        } else {
            writeFileOpenError(pathFileOpenErrorDoor1, formattedTime, "1");
        }
...
void MainWindow::on_btn_door_2_clicked()
{
    ...
    if (userName == nullptr) {
        ...
        QString dataFile = readFileOpenError(pathFileOpenErrorDoor2);
        ...
        writeFileOpenError(pathFileOpenErrorDoor2, formattedTime, count);

        if (date.secsTo(QDateTime::fromString(fileDateTime, "yyyy.MM.dd hh:mm:ss")) >= -30){
            writeFileOpenError(pathFileOpenErrorDoor2, formattedTime, count);

            if (count.toInt() > 3) {
                QString noti = "Thông báo (door_2)";
                QString text = "Có người đang mở cửa với thẻ không hợp lệ";

                QMessageBox::warning(this, noti, text);
            }
        } else {
            writeFileOpenError(pathFileOpenErrorDoor2, formattedTime, "1");
        }
